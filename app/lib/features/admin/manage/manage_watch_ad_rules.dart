...
/// Admin table for Watch Ads rules (§4). Each rule is a band of the day's ad
/// number → reward range and cooldown. Server validates + enforces.
final _watchAdRulesProvider =
    FutureProvider.autoDispose<List<Map<String, dynamic>>>((ref) {
  return ref.watch(adminRepositoryProvider).watchAdRules();
});

class ManageWatchAdRulesScreen extends ConsumerWidget {
  const ManageWatchAdRulesScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(_watchAdRulesProvider);
    return Scaffold(
      appBar: AppBar(title: const Text('Watch Ads Rules')),
      ...
      body: async.when(
        ...
        error: (e, _) => ErrorView(
            error: e, onRetry: () => ref.invalidate(_watchAdRulesProvider)),
        data: (rules) {
          if (rules.isEmpty) {
            return const EmptyView(
                icon: Icons.smart_display_rounded,
                title: 'No watch-ad rules',
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Expanded(
                          child: Text('Ads ${r['from_ad']}–${r['to_ad']}',
                          ...
                          onPressed: () async {
                            try {
                              await ref
                                  ...
                                  .deleteWatchAdRule(r['id'] as String);
                              ref.invalidate(_watchAdRulesProvider);
                            } catch (e) {
                              if (context.mounted) {
  ...
  Future<void> _edit(
    ...
    final from = TextEditingController(text: '${r?['from_ad'] ?? ''}');
    final to = TextEditingController(text: '${r?['to_ad'] ?? ''}');
        ...
        builder: (ctx, setState) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(r == null ? 'New watch-ad rule' : 'Edit watch-ad rule',
                    style: const TextStyle(
                        fontSize: 18, fontWeight: FontWeight.w800)),
                const SizedBox(height: 12),
                Row(children: [
                  Expanded(child: _num(from, 'Ads from')),
                  const SizedBox(width: 10),
                  Expanded(child: _num(to, 'Ads to')),
                ]),
    ...
    if (fromV == null || toV == null || fromV < 1 || toV < fromV) {
      if (context.mounted) {
        showSnack(context, 'Ad range invalid (from ≤ to, from ≥ 1)',
            error: true);
      }
      return;
    }
    ...
    try {
      await ref.read(adminRepositoryProvider).saveWatchAdRule({
        'id': r?['id'],
        'from_ad': fromV,
        'to_ad': toV,
        'min_reward': minV,
  ...
  static String _friendly(String e) {
    ...
    if (e.contains('INVALID_RANGE')) return 'Ad range is invalid.';
